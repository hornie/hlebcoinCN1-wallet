...
const char DEFAULT_WALLET_FILE_NAME[] = "hleb.wallet";
...
Settings::Settings() : m_p2pBindPort(0), m_cmdLineParser(nullptr) {
  m_defaultPoolList << "pool.hleb.cc:3333";
...
void Settings::init() {
  QFile cfgFile(getDataDir().absoluteFilePath("hlebwallet.cfg"));
...
bool Settings::isStartOnLoginEnabled() const {
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hlebwallet.plist");
  ...
  QString autorunFilePath = autorunDir.absoluteFilePath("hlebwallet.desktop");
  ...
  res = autorunSettings.contains("HLEBWallet") &&
    !QDir::fromNativeSeparators(autorunSettings.value("HLEBWallet").toString().split(' ')[0]).compare(QCoreApplication::applicationFilePath());
#endif
  return res;
}
...
void Settings::setStartOnLoginEnabled(bool _enable) {
  {
    ...
    autorunSettings.setValue("Label", "pw.hleb.hlebwallet");
    ...
    if (_enable) {
      ...
      autorunFile.write("Name=HLEB Wallet\n");
    ...
    if (_enable) {
      ...
      autorunSettings.setValue("HLEBWallet", appPath);
    } else {
      autorunSettings.remove("HLEBWallet");
    }
#endif
  }
...
void Settings::setUrlHandler() {
  QWriteLocker lock(&m_lock);
  QSettings protocolSettings("HKEY_CURRENT_USER\\Software\\Classes\\hleb", QSettings::NativeFormat);
  protocolSettings.setValue(".", "URL:hleb");
  protocolSettings.setValue("URL Protocol", "");
  QSettings iconSettings("HKEY_CURRENT_USER\\Software\\Classes\\hkeb\\DefaultIcon", QSettings::NativeFormat);
  iconSettings.setValue(".", QDir::toNativeSeparators(QCoreApplication::applicationFilePath()));
  QSettings openSettings("HKEY_CURRENT_USER\\Software\\Classes\\hleb\\shell\\open\\command", QSettings::NativeFormat);
...
void Settings::saveSettings() const {
  QFile cfgFile(QDir(m_cmdLineParser->getDataDir()).absoluteFilePath("hlebwallet.cfg"));
